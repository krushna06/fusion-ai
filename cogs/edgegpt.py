...
class EdgeGPT(Cog_Extension):
    ...
    @cookies_group.command(name="setting", description="can set personal fusionai cookies, no mandatory setting.")
    async def cookies_setting(self, interaction: discord.Interaction, *, cookies_file: discord.Attachment):
        ...
        try:
            if "json" in cookies_file.content_type or "text" in cookies_file.content_type:
                ...
                await interaction.followup.send("> **INFO：You have successfully set fusionai cookies!**")
                logger.info(f"\x1b[31m{interaction.user}\x1b[0m：setting fusionai cookies succeeded")
            else:
    ...
    # Chat with fusionai.
    @app_commands.command(name="fusionai", description="Create thread for conversation.")
